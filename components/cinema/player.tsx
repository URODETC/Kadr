...
import {highestSourceIndex,highestVideoTrack} from '@/lib/cinema/quality';
import {SkipPrompt} from './skip-prompt';
import {chapterSegments,mergeSegments,type SkipSegment} from '@/lib/cinema/segments';
...
export function Player({data,storageKey,watch,controls,title,loading,externalError,onProgress,onEnded,onNext,autoPlay=false}:Props){
 const menuId=useId();
 const [chapters,setChapters]=useState<{data:Playback;quality:number;segments:SkipSegment[]}|null>(null);
 ...
 const [error,setError]=useState(''),[recovery,setRecovery]=useState(''),[retry,setRetry]=useState(0),[selection,setSelection]=useState<{data:Playback;index:number}|null>(null),[menu,setMenu]=useState(false),[pending,setPending]=useState(false),[sync,setSync]=useState(''),[watched,setWatched]=useState(false),[ready,setReady]=useState(false),[resumeAt,setResumeAt]=useState(0);
 const quality=data?(selection?.data===data?selection.index:highestSourceIndex(data.sources)):0;
...
ui=new shaka.ui.Overlay(player,container.current!,element);ui.configure({controlPanelElements:['play_pause','episode_next','time_and_duration','spacer','mute','volume','episode_settings','overflow_menu','fullscreen'],overflowMenuButtons:['quality','language','captions','playback_rate','picture_in_picture'],seekBarColors:{base:'#ffffff30',buffered:'#ffffff60',played:'#d7ef75'},keyboardSeekDistance:10,seekOnTaps:true,tapSeekDistance:10,doubleClickForFullscreen:navigator.maxTouchPoints===0});ui.getControls()?.getLocalization()?.changeLocale(['ru']);player.addEventListener('error',event=>{if(disposed)return;const issue=playbackError((event as unknown as {detail:unknown}).detail);if(!issue)return;if(issue.fatal){setRecovery('');setError(issue.message);}else setRecovery(issue.message);});setInitialized(true);}catch(e){if(!disposed)setError(e instanceof Error?e.message:'Не удалось открыть плеер.');}})();
 ...
 useEffect(()=>{const player=engine.current,element=video.current;if(!initialized||!player||!element||!data||!ready)return;let disposed=false;setPending(true);setError('');setRecovery('');setChapters(null);
  void(async()=>{try{const s=data.sources[quality]||data.sources[0];player.configure({abr:{enabled:true}});await player.load(s.url,loadPosition.current||undefined,s.mime);if(disposed)return;void (async()=>{try{const track=player.getChaptersTracks()[0];if(!track)return;const values=await player.getChaptersAsync(track.language);if(!disposed)setChapters({data,quality,segments:chapterSegments(values)});}catch{/* Missing chapters must never block playback. */}})();const best=highestVideoTrack(player.getVideoTracks());if(best){player.configure({abr:{enabled:false}});player.selectVideoTrack(best,true);}for(const sub of data.subtitles){if(disposed)return;await player.addTextTrackAsync(sub.url,sub.lang,'subtitles',sub.mime,undefined,sub.label).catch(()=>{});}if(!disposed&&autoPlay)void element.play().catch(()=>{});}catch(e){if(!disposed){const issue=playbackError(e);if(issue)setError(issue.message);}}finally{if(!disposed)setPending(false);}})();
 ...
 return <div className="watch-player" ref={container} tabIndex={0} aria-label={title||'Видеоплеер'}>
  <video data-activity-title={watch?.title||title||""} ref={video} playsInline crossOrigin="anonymous" aria-label="Видео"/>
  {data&&<SkipPrompt key={`${storageKey}:${data.sources[quality]?.url}`} video={video} segments={mergeSegments(data.segments,chapters?.data===data&&chapters.quality===quality?chapters.segments:[])} hidden={menu||pending||!ready||!!error||!!externalError} onSkip={time=>{if(video.current)video.current.currentTime=time;setResumeAt(0);saveFn.current();}} onNext={onNext?()=>{if(endedRef.current)return;endedRef.current=true;saveFn.current('watched');onNext();}:undefined}/>}
  {menu&&<div id={menuId} ref={settingsRef} tabIndex={-1} role="region" aria-label="Настройки серии" className="watch-settings" onKeyDown={e=>{e.stopPropagation();if(e.key==='Escape'){e.preventDefault();closeMenu();}}}><div className="watch-settings-heading"><strong>Настройки серии</strong><button aria-label="Закрыть настройки" onClick={closeMenu}>×</button></div><div className="watch-setting-fields">{controls}{data&&<label>Качество<select value={quality} onChange={e=>{loadPosition.current=video.current?.currentTime||0;setSelection({data,index:Number(e.target.value)});}}>{data.sources.map((s,i)=><option key={i} value={i}>{s.quality?`${s.quality}p`:'Авто'}</option>)}</select></label>}</div><div className="watch-actions"><button disabled={!data} onClick={restart}>С начала</button><button disabled={!data} onClick={()=>watched?restart():saveFn.current('watched')}>{watched?'✓ Просмотрено — снять':'Отметить просмотренной'}</button><button onClick={closeMenu}>Готово</button></div></div>}
