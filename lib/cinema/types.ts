...
export type Playback = { sources: { url: string; quality: number; mime: string }[]; subtitles: { url: string; lang: string; label: string; mime: string; shift: number }[]; audios: { lang: string; label: string; original: boolean }[]; demo?: boolean };
