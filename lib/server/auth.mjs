...
export async function hashPassword(value,role='admin') {
 const minimum=role==='user'?6:12;
 if(typeof value!=='string'||value.length<minimum||value.length>128) throw new Error(`Пароль должен содержать от ${minimum} до 128 символов.`);
...
export async function createUser(name,password,role='user') {
 const normalized=username(name),hash=await hashPassword(password,role);
